...
const Navbar = () => {
  const navLinks = [
    { name: "Home", path: "/" },
    { name: "Monasteries", path: "/monastery" },
  ...
  // FIX: was calling navigate('/owner') inline which evaluates immediately
        ...
        {user ? (
          <UserButton>
            <UserButton.MenuItems>
              ...
              {isOwner && <UserButton.Action label="Owner Dashboard" labelIcon={BookIcon} onClick={() => navigate("/owner")} />}
            </UserButton.MenuItems>
          </UserButton>
        ) : (
      ...
      <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 z-50 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        {user && (
          <button className="border px-4 py-1 text-sm font-light rounded-full cursor-pointer"
            onClick={() => { handleOwnerClick(); setIsMenuOpen(false); }}>
            {isOwner ? "Dashboard" : "List Your Hotel"}
          </button>
        )}
        {user ? (
          <Link to="/myBookings" onClick={() => setIsMenuOpen(false)}
            className="border px-4 py-1 text-sm rounded-full">My Bookings</Link>
        ) : (
          <button onClick={openSignIn}
            className="bg-black text-white px-8 py-2.5 rounded-full cursor-pointer">Login</button>
        )}
      </div>
    </nav>
  );
